...
namespace LordG.IO
{
    public class BCSV
    {
        public BCSV(FileBase file)
        {
            ...
            mFields = new List<(int Key, Field Value)>();
            ...
            for (int i = 0; i < entryCount; i++)
            {
                ...
                foreach (Field f in mFields.Select(x => x.Value))
                {
        ...
        public Field AddField(string name, int offs, int type, int mask, int shift, object val)
        {
            ...
            if (offs == -1)
            {
                foreach(Field fe in mFields.Select(x => x.Value))
                {
        ...
        public void RemoveField(string name)
        {
            ...
            if (!(mFields.Where(x => x.Key == hash).ToArray().Length > 0))
                return;

            mFields.Remove(mFields.Where(x => x.Key == hash).First());
        ...
        public List<(int Key, Field Value)> mFields;
        public List<Entry> mEntries;


        public static Dictionary<int, string> sHashTable = new Dictionary<int, string>();
    }
}
